...
**This is a VS2022 toolchain wall, not a hard Pascal-vs-CUDA ceiling.**
Pascal (compute capability 6.0/6.1) is supported by NVIDIA across the
entire CUDA 12.x series, not just 12.1 — so a newer CUDA Toolkit paired
with VS2019 (not VS2022) might also work. That combination hasn't been
tested here; 12.1 + VS2019 is what's documented because it's what was
actually verified end-to-end on this hardware. If you try a newer
CUDA/VS2019 pairing and it works, that's a real data point worth reporting
back via an Issue.
...
- CUDA 12.1 + VS2019 is what's verified here — not necessarily the only
  working combination. See "The Core Problem" above.
